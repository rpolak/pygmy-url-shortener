...
class PygmyIntegrationTest(unittest.TestCase):
    ...
    def setup_class(cls):
        ...
        Client.header = {}
        cls.client = Client(
            cls.url, return_for_status=return_for_status, request_data_type="data"
        )
    ...
    def headers(self):
        _headers = {
            "Cookie": "csrftoken={}".format(self.token),
        ...
        if self.cookies is not None:
            ...
            _headers["Cookie"] += "; " + "; ".join(
                ["{}= {}".format(k, v) for k, v in co_di.items()]
            )
        return _headers
    ...
    def test_signup(self):
        ...
        response = requests.post(
            self.url + "/signup", data=self.user_data, headers=self.headers
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_invalid_email_login(self):
        ...
        response = requests.post(
            self.url + "/login", data=login_data, headers=self.headers
        )
        self.assertEqual(response.status_code, 404)
        self.assertTrue("DASHBOARD" not in response.text)
        self.assertTrue("LOGIN" in response.text)
        self.assertTrue(
            "No user found with email: {}".format(login_data["email"]) in response.text
        )
    ...
    def test_login(self):
        ...
        and dashboard table"""
        response = requests.post(
            self.url + "/login", data=self.login_data, headers=self.headers
        )
        self.assertEqual(response.status_code, 200)
